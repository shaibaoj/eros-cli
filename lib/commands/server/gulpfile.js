...
const webpackInstance = require('./newPack.config.js')
const fs = require('fs')
...
gulp.task('write-app', (done) => {
    if(argv.p){
        let appName = argv.p;
        let android_Market = false
        let app_cache = false

        if(argv.m){
            android_Market = argv.m;
        }
        if(argv.c){
            app_cache = argv.c;
        }
...
export default {
    ...
    'cache':${app_cache},
        ...
        `
        fs.writeFileSync(path.resolve(process.cwd(), 'src/js/')+"/config/app.js", app_js, 'utf8');
    }
    done()
});
...
gulp.task('dev', (callback) => {
    gulpSequence('clean','write-app', ['build-assets', 'build-iconfont', 'build-mock', 'build-js'], ['start-mock', 'handle-config-native', "compatible-weex-debug", 'watch', 'socket'])(callback)
});

gulp.task('weex', (callback) => {
    gulpSequence('clean','write-app', 'weex:js')(callback)
});
...
gulp.task('weex-eros:ios', (callback) => {
    gulpSequence('clean','write-app', 'weex-eros-ios', 'handle-config-native', 'weex:js')(callback)
});
gulp.task('weex-eros:android', (callback) => {
    gulpSequence('clean','write-app', 'weex-eros-android', 'handle-config-native', 'weex:js')(callback)
});
gulp.task('weex-eros:all', (callback) => {
    gulpSequence('clean','write-app', 'weex-eros-all', 'handle-config-native','weex:js')(callback)
});
